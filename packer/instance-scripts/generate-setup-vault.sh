...
vault policy-write tx_executor \$TX_EXECUTOR_POLICY
vault policy-write eximchain_node \$EXIMCHAIN_NODE_POLICY

# Write policy to the roles used by instances
vault write auth/aws/role/$TX_EXECUTOR_ROLE_NAME auth_type=iam policies=tx_executor bound_iam_principal_arn=arn:aws:iam::$AWS_ACCOUNT_ID:role/$TX_EXECUTOR_ROLE_NAME
