...
module "cert_tool" {
  ...
  ca_common_name          = "tx-executor-vault cert authority"
  common_name             = "tx-executor cert network"
  dns_names               = ["${aws_lb.tx_executor_vault.dns_name}"]
...
resource "aws_s3_bucket" "vault_certs" {
  bucket_prefix = "tx-executor-vault-certs-"
  acl           = "private"
}
...
resource "aws_iam_server_certificate" "vault_certs" {
  name_prefix       = "tx-executor-vault-cert-"
