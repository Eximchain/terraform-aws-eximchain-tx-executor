...
resource "aws_autoscaling_group" "vault_cluster" {
  ...
  tags = [
    {
      key                 = "Name"
      value               = "transaction-executor-vault"
...
resource "aws_security_group" "vault_cluster" {
  name_prefix = "transaction-executor-vault-"
  description = "Security group for the transaction-executor-vault launch configuration"
  vpc_id      = "${var.aws_vpc}"
...
resource "aws_s3_bucket" "tx_executor_vault" {
  bucket_prefix = "tx-executor-vault-"
...
data "aws_iam_policy_document" "vault_s3" {
  statement {
    ...
    resources = [
      "${aws_s3_bucket.tx_executor_vault.arn}",
      "${aws_s3_bucket.tx_executor_vault.arn}/*",
    ]
  }
}
